...
void t_dbtrigger_i2c(s_data_t *data) {
	...
	if (PQstatus(conn_hvac) != CONNECTION_OK) {
		memset(printbuff, 0, SIZE_PRINTBUFF);
		sprintf(printbuff, "exiting on error, connecting to hvac database in 't_dbtrigger_i2c' failed: %s", PQerrorMessage(conn_hvac));
		do_print(data, printbuff, RED);
		PQfinish(conn_hvac);
	...
	while(!data->error){
		...
		while ((notify = PQnotifies(conn_arduino)) != NULL) {
			...
			res1 = PQexec(conn_arduino, sqlbuff);
        			...
        			//pthread_mutex_lock(pmut_sql_hvac);
        			memset(sqlbuff, 0, sizeof(sqlbuff));
	        		sprintf(sqlbuff, "update sensor set state_raw = '%.2f' where unit = '%s' and port = 'i%02d'", atof(tbuff), PQgetvalue(res1, 0, 1), k);
